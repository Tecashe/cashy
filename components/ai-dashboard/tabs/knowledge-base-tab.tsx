...
export function KnowledgeBaseTab({ onUpdate }: { onUpdate?: () => void }) {
  ...
  const handleDocumentUpload = async (urls: string[]) => {
    try {
      for (const url of urls) {
        console.log("[KnowledgeBase] Importing document from:", url)
        const response = await fetch(url)
        ...
        // Extract filename from URL as a default title if none exists
      ...
      toast({
        title: "Document loaded",
        description: "Content successfully extracted and added to form.",
      })

      setDocumentUploadMode("manual")
    } catch (error) {
      console.error("[KnowledgeBase] Import error:", error)
      toast({
        title: "Import Error",
        description: "Could not read the document content. Try manual entry.",
        variant: "destructive",
      })
    }
  }
            ...
            <div className="space-y-2">
              ...
              <Tabs value={documentUploadMode} onValueChange={(v) => setDocumentUploadMode(v as "manual" | "upload")}>
                ...
                <TabsContent value="upload">
                  ...
                  />
                  <p className="text-xs text-muted-foreground mt-2">
                    Upload a text file, PDF, CSV, or JSON document. The content will be extracted and added to the
                    knowledge base.
                  </p>
                </TabsContent>
              </Tabs>
