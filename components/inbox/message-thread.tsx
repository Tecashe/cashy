...
import { CheckCheck, Check, ImageIcon, Mic, Video } from "lucide-react"

type Message = {
  id: string
  content: string
  createdAt: Date
  sender: "business" | "participant" // Use sender instead of isFromUser
  isRead: boolean
  messageType?: string
  attachmentUrl?: string | null
  attachmentMetadata?: any
...
export function MessageThread({ messages, participantAvatar, participantName, businessAvatar }: MessageThreadProps) {
  const bottomRef = useRef<HTMLDivElement>(null)
  const [prevMessageCount, setPrevMessageCount] = useState(messages.length)
  const [shouldAutoScroll, setShouldAutoScroll] = useState(true)
  const scrollContainerRef = useRef<HTMLDivElement>(null)
    ...
    switch (messageType) {
      case "image":
        return (
          <div className="space-y-2">
            {message.attachmentUrl && (
              <div className="rounded-lg overflow-hidden bg-muted/30 max-w-sm">
                <img
                  src={message.attachmentUrl}
                  alt="Shared image"
      ...
      case "video":
        return (
          <div className="space-y-2">
            {message.attachmentUrl && (
              <div className="rounded-lg overflow-hidden bg-muted/30 relative group max-w-sm">
                <video
                  src={message.attachmentUrl}
                  controls
      ...
      case "audio":
        return (
          <div className="space-y-2">
            {message.attachmentUrl && (
              <div className="flex items-center gap-2 p-3 bg-muted/30 rounded-lg min-w-[200px]">
                <Mic className="h-4 w-4 text-muted-foreground shrink-0" />
                <audio src={message.attachmentUrl} controls className="flex-1 h-8">
                  Your browser does not support the audio tag.
                </audio>
              </div>
            )}
  ...
  return (
    <div 
      ...
      className="flex-1 overflow-y-auto px-3 md:px-6 py-4 bg-gradient-to-b from-background to-muted/10"
    >
      <div className="space-y-6 max-w-4xl mx-auto">
        {Object.entries(groupedMessages).map(([date, msgs]) => (
          <div key={date}>
            <div className="flex items-center gap-4 mb-6">
              <div className="flex-1 h-px bg-border" />
              <span className="text-xs text-muted-foreground font-medium px-3 py-1.5 bg-muted/50 rounded-full shadow-sm">
                {formatDateLabel(date)}
              </span>
              <div className="flex-1 h-px bg-border" />
            </div>
              ...
              {msgs.map((message, index) => {
                ...
                return (
                  <div
                    key={message.id}
                    className={cn(
                      "flex gap-2 md:gap-3 items-end group transition-opacity hover:opacity-100",
                      isFromBusiness && "flex-row-reverse",
                      isConsecutive && "opacity-90",
                    )}
                  >
                    {showAvatar ? (
                      <Avatar className="h-7 w-7 md:h-8 md:w-8 shadow-sm shrink-0 ring-2 ring-background">
                        {isFromBusiness ? (
                          <>
                            <AvatarImage src={businessAvatar || undefined} />
                            <AvatarFallback className="bg-primary text-primary-foreground text-xs font-medium">
                              {getInitials("You")}
                            </AvatarFallback>
                          </>
                    ...
                    <div className={cn("flex flex-col gap-1 max-w-[75%] md:max-w-[65%]", isFromBusiness && "items-end")}>
                      <div
                        className={cn(
                          "rounded-2xl px-3 md:px-4 py-2 md:py-2.5 shadow-sm transition-all",
                          isFromBusiness
                            ? "bg-primary text-primary-foreground rounded-br-sm"
                            : "bg-muted text-foreground rounded-bl-sm",
                          showAvatar && isFromBusiness && "rounded-tr-2xl",
                          showAvatar && !isFromBusiness && "rounded-tl-2xl",
                        )}
                      >
                        {renderMessageContent(message)}
                      </div>
