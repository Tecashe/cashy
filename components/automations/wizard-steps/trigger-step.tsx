...
//   const triggerExists = (type: TriggerType): boolean => {
//     return flow.triggers.some((trigger) => trigger.type === type)
//   }

//   const handleAddTrigger = (type: TriggerType) => {
//     if (triggerExists(type)) {
//       const triggerInfo = TRIGGER_TYPES[type]
//       // Allow duplicates only if the trigger requires configuration (so users can have different configs)
//       if (!triggerInfo.requiresConfig) {
//         alert(`You already have a "${triggerInfo.label}" trigger. You cannot add the same trigger twice.`)
//         setShowSelector(false)
//         return
//       }
//     }
...
//                             trigger.config?.storyIds &&
//                             trigger.config.storyIds.length > 0 && (
//                               <Badge variant="secondary" className="text-xs shadow-sm">
//                                 {trigger.config.storyIds.length}{" "}
//                                 {trigger.config.storyIds.length === 1 ? "story" : "stories"} selected
//                               </Badge>
//                             )}
//                           {(trigger.type === "comment" || trigger.type === "keyword") &&
//                             trigger.config?.keywords &&
...
//         existingTriggers={flow.triggers.map((t) => t.type)}
//       />
...
//                               size="sm"
//                               onClick={() => setEditingTrigger(index)}
//                               className="bg-primary/10 font-medium hover:bg-primary/20 hover:text-primary border border-primary/20"
//                             >
//                               Configure
//                             </Button>
//                           )}
...
//           onSave={(trigger) => handleUpdateTrigger(editingTrigger, trigger)}
//           accounts={accounts}
//           selectedAccountId={flow.instagramAccountId}
//         />
//       )}
//     </div>
//   )
...
// export function TriggerStep({ flow, setFlow, accounts, userTier = "free" }: TriggerStepProps) {
//   const [showSelector, setShowSelector] = useState(false)
//   const [editingTrigger, setEditingTrigger] = useState<number | null>(null)
...
//   const canAddTrigger = (type: TriggerType): boolean => {
//     return isFeatureAvailable(userTier, "trigger", type)
//   }

//   const handleAddTrigger = (type: TriggerType) => {
//     if (!canAddTrigger(type)) {
//       const requiredTier = getMinimumTierForFeature("trigger", type)
//       alert(`This trigger requires ${requiredTier} plan. Please upgrade to use it.`)
//       return
//     }
...
//       // Show beautiful upgrade modal instead of browser alert
//       setUpgradeModal({
//         open: true,
//         triggerType: type,
//       })
...
//         {upgradeModal.open && upgradeModal.triggerType && (
//           <UpgradeModal
//             open={upgradeModal.open}
//             onClose={() => setUpgradeModal({ open: false, triggerType: null })}
//             featureName={TRIGGER_TYPES[upgradeModal.triggerType].label}
//             requiredTier={getMinimumTierForFeature("trigger", upgradeModal.triggerType)}
//             benefits={getTriggerBenefits(
//               upgradeModal.triggerType,
//               getMinimumTierForFeature("trigger", upgradeModal.triggerType)
//             )}
//           />
//         )}
//       </div>
...
export function TriggerStep({ flow, setFlow, accounts, userTier }: TriggerStepProps) {
  const [showSelector, setShowSelector] = useState(false)
  const [editingTrigger, setEditingTrigger] = useState<number | null>(null)
  const [upgradeModal, setUpgradeModal] = useState<UpgradeModalState>({
    open: false,
  ...
  // Debug logging
  console.log("=== TriggerStep Debug ===")
  console.log("userTier prop received:", userTier)
  console.log("userTier type:", typeof userTier)
  console.log("========================")
  ...
  const canAddTrigger = (type: TriggerType): boolean => {
    const canAdd = isFeatureAvailable(userTier, "trigger", type)
    console.log(`🔍 Can add trigger "${type}":`, {
      userTier,
      canAdd,
      featureType: "trigger"
    })
  ...
  const handleAddTrigger = (type: TriggerType) => {
    console.log(`🔍 handleAddTrigger called for: ${type}`)
      ...
      <div className="space-y-6">
        ...
        {process.env.NODE_ENV === "development" && (
          <div className="rounded-lg bg-muted p-3 text-xs">
            <strong>Debug:</strong> Current tier: {userTier}
          </div>
        )}
