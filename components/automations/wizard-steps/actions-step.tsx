...
// export function ActionsStep({ flow, setFlow, tags }: ActionsStepProps) {
//   console.log("[v0] ActionsStep - ACTION_TYPES:", ACTION_TYPES)
//   console.log("[v0] ActionsStep - flow:", flow)
//   console.log("[v0] ActionsStep - flow.actions:", flow.actions)

//   const [showSelector, setShowSelector] = useState(false)
//   const [editingAction, setEditingAction] = useState<number | null>(null)

//   const handleAddAction = (type: ActionType) => {
//     const hasDuplicate = flow.actions.some((action) => action.type === type)
//     if (hasDuplicate) {
//       const confirmAdd = window.confirm(
//         `You already have a "${ACTION_TYPES[type].label}" action. Are you sure you want to add another one?`,
//       )
//       if (!confirmAdd) {
//         setShowSelector(false)
//         return
...
//                   const actionInfo = ACTION_TYPES?.[action.type]
//                   if (!actionInfo) {
//                     console.error("[v0] Action type not found:", action.type)
//                     return null
//                   }
//                   const ActionIcon = actionInfo.icon
...
//         description: `You already have a "${ACTION_TYPES[type].label}" action. Are you sure you want to add another one?`,
...
//                   Build the sequence of actions that will be executed
//                 </CardDescription>
//               </div>
//             </div>
//           </CardHeader>
...
//     return isFeatureAvailable(userTier, "action", type)
//   }
...
export function ActionsStep({ flow, setFlow, tags, userTier}: ActionsStepProps) {
  const [showSelector, setShowSelector] = useState(false)
  const [editingAction, setEditingAction] = useState<number | null>(null)
  const [upgradeModal, setUpgradeModal] = useState<UpgradeModalState>({
    open: false,
  ...
  // Debug log to verify tier is being passed correctly
  console.log("ActionsStep - Current user tier:", userTier)

  const canAddAction = (type: ActionType): boolean => {
    const canAdd = isFeatureAvailable(userTier, "action", type)
    console.log(`Can add action ${type}:`, canAdd, "for tier:", userTier)
    return canAdd
  }
  ...
  const handleAddAction = async (type: ActionType) => {
    if (!canAddAction(type)) {
      const requiredTier = getMinimumTierForFeature("action", type)
      console.log(`Action ${type} requires tier:`, requiredTier, "but user has:", userTier)
