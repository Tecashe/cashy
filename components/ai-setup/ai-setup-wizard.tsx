...
export function AISetupWizard({
  ...
  const handleStepComplete = async (stepId: string, data: any) => {
    try {
      if (stepId === "business") {
        const { saveBusinessInfo } = await import("@/actions/ai-setup-actions")
        await saveBusinessInfo(data)
        toast({ title: "Business info saved successfully" })
      } else if (stepId === "products") {
        const { saveProducts } = await import("@/actions/ai-setup-actions")
        await saveProducts(data.products || [])
        toast({ title: "Products saved successfully" })
      } else if (stepId === "knowledge") {
        const { saveKnowledgeDocuments } = await import("@/actions/ai-setup-actions")
        await saveKnowledgeDocuments(data.documents || [])
        toast({ title: "Knowledge base saved successfully" })
      } else if (stepId === "integrations") {
        toast({ title: "Integrations configured successfully" })
      } else if (stepId === "ai-config") {
        const { saveAIConfiguration } = await import("@/actions/ai-setup-actions")
        const result = await saveAIConfiguration({
          automationId: currentAutomationId,
          instagramAccountId,
          automationName: data.automationName || "AI Auto-Reply",
          aiConfig: data,
        })
        setCurrentAutomationId(result.automationId)
        toast({ title: "AI configuration saved successfully" })
      } else if (stepId === "test") {
        if (currentAutomationId) {
          const { completeAISetup } = await import("@/actions/ai-setup-actions")
          await completeAISetup(currentAutomationId)
          toast({ title: "AI assistant is now live!", description: "Your automation is active" })
        }
      }
