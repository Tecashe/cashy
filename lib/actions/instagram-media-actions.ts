...
export async function getInstagramStories(accountId: string): Promise<InstagramMediaItem[]> {
  ...
  try {
    ...
    console.log("[Instagram Stories] Calling Instagram Stories API...")
    const storiesData = await api.getStories()
    
    console.log("[Instagram Stories] Stories API response:", JSON.stringify(storiesData, null, 2))
    console.log("[Instagram Stories] Number of stories returned:", storiesData.data?.length || 0)

    if (!storiesData.data || storiesData.data.length === 0) {
      console.log("[Instagram Stories] No stories returned from API")
      return []
    }

    const stories = storiesData.data.map((story: any) => ({
      id: story.id,
      caption: story.caption || "Story",
    ...
    console.log("[Instagram Stories] Processed stories count:", stories.length)
    console.log("[Instagram Stories] Stories data:", JSON.stringify(stories, null, 2))
...
export async function getInstagramMedia(
  ...
  try {
    ...
    if (options?.type === "stories") {
      const storiesData = await api.getStories()
      return (storiesData.data || []).map((media: any) => ({
        id: media.id,
        caption: media.caption || "Story",
        media_type: media.media_type,
        media_url: media.media_url,
